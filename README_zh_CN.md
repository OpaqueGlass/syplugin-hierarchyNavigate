...
> 当前版本：v1.0.3（临时修复）**修复**：移动端闪卡面包屑不能整除显示的问题；
> 上一版本：v1.0.2（此版本为测试版） **修复**：一些移动端显示问题；**新增**：🧪“往年今日”内容区；**改进**：🧪日记的上一篇下一篇跳转；**改进**：一些样式调整；**改进**：提升部分情况下的响应速度；
>
> 此版本加入并默认启用链接等宽分列功能，若使用代码片段修改过样式，请将分列数调整为0以禁用分列并关注更新日志中其他更改。
> 
